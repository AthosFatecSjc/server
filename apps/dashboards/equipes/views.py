...
def dashboard_equipes(request):
    """View do dashboard de equipes - agora muito mais limpa e segura"""

    if request.method != "GET":
        return render(
            request, "equipes/error.html", {"error": "Método não permitido"}, status=405
        )
    ...
    try:
        ...
        dados_grafico = DashboardEquipesService.gerar_dados_grafico_horas(
            registros_horas
        )
        ...
        devs_selected_count = (
            DashboardEquipesService.contar_desenvolvedores_selecionados(
